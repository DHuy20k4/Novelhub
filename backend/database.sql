...
CREATE TABLE Follows (
    ...
    CONSTRAINT CHK_Follows_NoSelfFollow 
...
CREATE TABLE Stories (
    ...
    moderationStatus NVARCHAR(20) NOT NULL DEFAULT 'draft'
        CHECK (moderationStatus IN 
        ('draft', 'pending', 'approved', 'rejected', 'locked')),

    writingStatus NVARCHAR(20) NOT NULL DEFAULT 'ongoing'
        CHECK (writingStatus IN 
...
CREATE TABLE StoryCategories (
    ...
    CONSTRAINT FK_StoryCategories_Stories
        FOREIGN KEY (storyId) REFERENCES Stories(id)
        ON DELETE CASCADE,

    CONSTRAINT FK_StoryCategories_Categories
...
CREATE TABLE Chapters (
    ...
    CONSTRAINT FK_Chapters_Stories
        FOREIGN KEY (storyId) REFERENCES Stories(id)
        ON DELETE CASCADE
)
GO
...
CREATE TABLE ReadingHistory (
    ...
    CONSTRAINT UQ_ReadingHistory_User_Story
        UNIQUE (userId, storyId),

    CONSTRAINT FK_ReadingHistory_Users
        FOREIGN KEY (userId) REFERENCES Users(id),

    CONSTRAINT FK_ReadingHistory_Stories
        FOREIGN KEY (storyId) REFERENCES Stories(id),

    CONSTRAINT FK_ReadingHistory_Chapters
...
CREATE TABLE Reviews (
    ...
    CONSTRAINT UQ_Reviews_User_Story
        UNIQUE (userId, storyId),

    CONSTRAINT FK_Reviews_Users
        FOREIGN KEY (userId) REFERENCES Users(id),

    CONSTRAINT FK_Reviews_Stories
        FOREIGN KEY (storyId) REFERENCES Stories(id)
        ON DELETE CASCADE
)
GO
...
CREATE TABLE Comments (
    ...
    CONSTRAINT CHK_Comments_NoSelfParent
        CHECK (id <> parentId),
...
---------------------------------------------------
-- STORY VIEW LOGS (THIẾU TRONG SQL CỦA ÔNG)
---------------------------------------------------
CREATE TABLE StoryViewLogs (
    id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
    ...
    CONSTRAINT UQ_StoryViewLogs_Story_Date
        UNIQUE (storyId, viewDate),

    CONSTRAINT FK_StoryViewLogs_Stories
        FOREIGN KEY (storyId) REFERENCES Stories(id)
        ON DELETE CASCADE
)
GO